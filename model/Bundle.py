...
class Bundle:
    ...
    @classmethod
    def getByIcon(cls, server_id: int, bundle_icon: str) -> "Bundle|None":
        cursor = None
        try:
            sql = "SELECT * FROM bundles WHERE `server_id` = ? AND `bundle_icon` = ?"
            cursor = conn.cursor()
            cursor.execute(sql, (server_id, bundle_icon))
